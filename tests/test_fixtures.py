...
routing the classifier's output should produce. They are deliberately:
...
def run_fixture(name: str) -> tuple[int, int, int, int, str]:
    """Run one fixture through `process --dry-run`; return (exit, hits, maybes, news, out)."""
    ...
    out = proc.stdout
    # Tier markers printed by cmd_process: "  [HIT ] ", "  [MAYBE] ", "  [NEWS] ".
    hits = out.count("[HIT ]")
    maybes = out.count("[MAYBE]")
    news = out.count("[NEWS]")
    return proc.returncode, hits, maybes, news, out
...
def main() -> int:
    ...
    failures = 0
    for name, (exp_exit, exp_high, exp_maybe, exp_news) in CASES.items():
        if not os.path.exists(os.path.join(FIXTURES, name)):
            print(f"FAIL  {name}: fixture missing")
            failures += 1
            continue
        code, hits, maybes, news, out = run_fixture(name)
        problems = []
        if code != exp_exit:
        ...
        if news != exp_news:
            problems.append(f"NEWS {news} != {exp_news}")
        if problems:
            failures += 1
            ...
            print(f"ok    {name}: exit={code} high={hits} maybe={maybes} news={news}")
